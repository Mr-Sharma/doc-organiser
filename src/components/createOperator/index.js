...
function CreateOperatorsPage (props) {
  ...
  const getUsers = async () => {
    try {
      let response = await axios.get('/api/user/fetchAllUsers/1');
      ...
      setUsers([]);
      setUnfilteredUsers([]);
      alert.error("error in fetching data entry users")
    }
  }

  const openPopup=()=> {
    document.getElementById('createDataEntryPopup').style.display = 'block';
  }

  const hidePopUp=()=> {
    document.getElementById('createDataEntryPopup').style.display = 'none';
    setSelectedUser({})
  }
  ...
  const handleSubmit = (e) => {
    ...
    if(popupType == 'create' && username!=='' && username!==undefined && phoneNumber!=='' && phoneNumber!==undefined){
      ...
      .then(response => {
          alert.success("Data Entry user created successfully")
          setShowErrorMsg(false);
          getUsers()
          hidePopUp();
      })
    ...
    if(username!=='' && username!==undefined && phoneNumber!=='' && phoneNumber!==undefined){
      ...
      .then(response => {
        if(!response.data.success){
          ...
          alert.error("error in updating data entry user")
          setShowErrorMsg(true);
        } else {
          alert.success("Data entry user updated successfully")
          setShowErrorMsg(false);
          getUsers()
          hidePopUp();
        }
  ...
  const handleDelete = async (user) => {
    if(user._id!=='' && user._id!==undefined){
      ...
      .then(response => {
        if(!response.data.success){
          alert.error("error in deleting data entry user");
        } else {
          alert.success("Data Entry user deleted successfully");
          getUsers();
        }
      })
      .catch(error => {      
  ...
  return (
    <div>
      <div className='doc-card'>
       ...
       {/*popup to data entry creation*/}
       <div id="createDataEntryPopup" className="nj-overly add-rebound-animation" >
...
export default CreateOperatorsPage;